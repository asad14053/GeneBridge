...
def plot_umaps(
    ...
    fig.suptitle(
        (
            f"{method_label} cell-population preservation across "
    ...
    png = (
        OUT_DIR
        / f"{METHOD}_shared_umap_celltype.png"
    )

    pdf = (
        OUT_DIR
        / f"{METHOD}_shared_umap_celltype.pdf"
    )
...
def main():
    ...
    print(
        f"{method_label} SHARED-BASIS UMAP"
    )
    ...
    coord_file = (
        OUT_DIR
        / f"{METHOD}_shared_umap_coordinates.csv"
    )
