...
def plot_fold_metric_summary(
    ...
    *,
    ...
    row = fold_summary.iloc[0]
    fig, axes = plt.subplots(1, 2, figsize=(14, 5.5))

    for ax, metrics, panel_title in [
        (axes[0], HIGHER_IS_BETTER, "Higher is better"),
        (axes[1], LOWER_IS_BETTER, "Lower is better"),
    ]:
        labels = [METRIC_LABELS[m] for m in metrics]
        values = [float(row[m]) for m in metrics]
        bars = ax.bar(np.arange(len(metrics)), values)
        ax.set_xticks(np.arange(len(metrics)), labels, rotation=25, ha="right")
...
def plot_three_fold_metrics(
    ...
    *,
    ...
    fold_summary = fold_summary.sort_values("fold")
    fig, axes = plt.subplots(2, 4, figsize=(18, 9))
    for ax, metric in zip(axes.ravel(), FOLD_METRIC_COLUMNS):
        x = fold_summary["fold"].astype(int).to_numpy()
        y = fold_summary[metric].astype(float).to_numpy()
        bars = ax.bar(x, y)
        ax.set_xticks(x, [f"Fold {fold}" for fold in x])
