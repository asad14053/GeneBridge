...
"""
Paired statistical comparisons among imputation Experiments 5, 5.1 and 5.3.
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Paired t-test and TOST comparison across "
    ...
    parser.add_argument(
        "--model",
        required=True,
        help="Model directory key, e.g. vista, gimvi, tangram.",
    )

    parser.add_argument(
        "--model-label",
        required=True,
        help="Display label, e.g. VISTA or gimVI.",
    )

    parser.add_argument(
        "--combined-dir",
        default="combined_v2",
        help="Aggregated result directory name.",
    )
...
def find_metric_file(
    ...
    combined_dir = experiment_dir / COMBINED_DIR_NAME
...
def create_forest_plot(
    ...
    axis.set_title(
        f"{MODEL_LABEL} paired TOST comparisons with uncertainty distributions\n"
...
def write_text_report(
    ...
    lines = [
        f"{MODEL_LABEL} PAIRED STATISTICAL COMPARISON REPORT",
        "=" * 110,
        "",
...
def main() -> None:
    ...
    MODEL_LABEL = str(args.model_label).strip()
    COMBINED_DIR_NAME = str(args.combined_dir).strip()
    ...
    if not COMBINED_DIR_NAME:
        raise ValueError("--combined-dir cannot be empty.")
    ...
    section(f"LOADING {MODEL_LABEL} METRICS")
    ...
    gene_results_file = (
        RESULT_DIR
        / f"{MODEL}_gene_level_paired_ttest_tost.csv"
    )

    fold_results_file = (
        RESULT_DIR
        / f"{MODEL}_fold_level_nmi_ari_paired_ttest_tost.csv"
    )

    pairing_file = (
        RESULT_DIR
        / f"{MODEL}_pairing_audit.csv"
    )

    source_manifest_file = (
        RESULT_DIR
        / f"{MODEL}_statistical_test_source_manifest.json"
    )

    text_report_file = (
        RESULT_DIR
        / f"{MODEL}_paired_test_report.txt"
    )

    plot_file = (
        RESULT_DIR
        / f"{MODEL}_gene_level_relative_difference_forest.png"
    )
